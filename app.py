...
def run_water_simulation(original_height_map_raw, dx, dy, rainfall_type_code, simulation_duration_steps):
    ...
    # 這裡修正：無論細化與否，每個物理面積單元接收的降雨深度應該是相同的。
    # 因為 base_rainfall_height_per_min 已經是每單位面積的深度。
    rainfall_per_expanded_grid_cell_per_min = base_rainfall_height_per_min 

    # 計算細化後的實際網格間距，用於前端計算總體積
    dx_expanded = dx / effective_subdivision_factor
    dy_expanded = dy / effective_subdivision_factor
    ...
    return initial_terrain_for_frontend, simulation_snapshots_for_frontend, dx_expanded, dy_expanded
...
global_dx_expanded = 0 # 新增：儲存擴展後的 dx
global_dy_expanded = 0 # 新增：儲存擴展後的 dy
    ...
    if file and file.filename.endswith(('.xlsx', '.xls')): # 支援 .xls
        try:
            ...
            return {
                ...
                "rows": rows_expanded,
                "cols": cols_expanded,
                "dx": dx, # 這些 dx, dy 用於前端 Three.js 空間縮放 (原始間距)
                "dy": dy,
                "dx_expanded": global_dx_expanded, # 新增：傳遞擴展後的實際網格間距
                "dy_expanded": global_dy_expanded, # 新增：傳遞擴展後的實際網格間距
...
def get_water_data():
    ...
    if global_sim_current_step < len(global_simulation_snapshots):
        ...
        return {
            ...
            "total_steps": len(global_simulation_snapshots),
            "dx_expanded": global_dx_expanded, # 新增：每次都傳遞擴展後的實際網格間距
            "dy_expanded": global_dy_expanded  # 新增：每次都傳遞擴展後的實際網格間距
        }
    else:
